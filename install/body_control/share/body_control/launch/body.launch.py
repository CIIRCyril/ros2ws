...
def generate_launch_description():
    ...
    bag_package_directory = f"{get_package_share_directory('utils')}/../../lib/utils/bag_record"
    bag = ExecuteProcess(
        cmd=[f'{bag_package_directory}/scripts/init.sh'],
        output='screen'
    )

    utils_package_directory = f"{get_package_share_directory('utils')}/../../lib/utils/disk_manager"
    disk_manager = ExecuteProcess(
        cmd=[f'{utils_package_directory}/scripts/init.sh'],
        output='screen'
    )
    ...
    diagnose_container = ComposableNodeContainer(
        name='diagnose_tiangong2_pro_container',
        namespace='',
        package='rclcpp_components',
        executable='component_container_mt',
        composable_node_descriptions=[
            ComposableNode(
                ...
                extra_arguments=[{'use_intra_process_comms': True}],
                parameters = [
                    { "xml_config_file": "param/tiangong2.0_pro.xml" }
                ],
            )
        ],
        output='screen',
    ...
    power_board_container = ComposableNodeContainer(
    ...
    return LaunchDescription([componentContainer, diagnose_container, power_board_container, bag, n1, disk_manager])
